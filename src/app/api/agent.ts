...
const createAxiosInstance = (baseUrl: string) => {
    ...
    return {
        get: async <T>(url: string, params?: URLSearchParams) => instance
            .get<T>(url, { params })
            .then(responseData),

        getPaginated: async <T>(url: string, params?: URLSearchParams) => instance
            .get<T>(url, { params })
            .then(paginationResponseData),

        post: async <T>(url: string, body: object) => instance
            .post<T>(url, body)
            .then(responseData),

        put: async <T>(url: string, body: object) => instance
            .put<T>(url, body)
            .then(responseData),

        delete: async <T>(url: string) => instance
            .delete<T>(url)
            .then(responseData),
    };
};
